...
* **스프링이 제공하는 다양한 스코프들**
  ...
  * 프로토타입 : 스프링 컨테이너가 빈을 생성하고 의존 관계를 주입한 이후에는 더는 관리하지 않는 짧은 범위의 스코프
  * 웹 관련 스코프 : 
    * "**request**" : 웹 요청이 들어오가 나갈 때 까지 유지되는 스코프
    * "**session**" : 웹 세션이 생성되고 종료될 때 까지 유지디는 스코프
    * "**application**" : 웹의 서플릿 컨텍스트와 같은 범위를 가진다.
...
### [프로토타입 스코프]
**싱글톤 스코프**는 스프링 컨테이너가 종료된 때까지 유지된다. 또한 클라이언트가 빈을 요청하면 항상 같은 인스턴스를 반환한다.반면에 프로토타입 스코프는 
반면에 **프로토타입 스코프**는 클라이언트의 요청 시에 **항상 새로운 빈을 생성하고 의존 관계를 주입한 후 빈을 반환해 주고, 더이상 관리하지 않는다**.


&nbsp;
**핵심은, 스프링 컨테이너는 프로토타입 빈을 생성하고, 의존 관계를 주입한 후에 초기화 처리**까지만 한다는 점이다.

**따라서,** 초기화 이후에 프로토타입 빈을 관리할 책임은 "클라이언트"에게 있다.  => 소멸 콜백 매서드 호출 X

&nbsp;

대부분은 싱글톤 스코프를 사용하지만 만약 프로토파입 스코프와 싱글톤 스코프와 함께 사용된다면???

&nbsp;

**[프로토타입 스코프와 싱글톤 스코프를 함께 사용했을 경우]**

* 싱글톤 스코프는 클라이언트에게 항상 같은 인스턴스를 반환하고, 
* 프로토타입 스코프는 클라이언트의 요청이 있을 경우 항상 새로운 빈을 생성해서 반환한다고 했다.

여기서 사실 중요한 문제가 생길 수 있는데, 
**이는 싱글톤을 사용하는 빈이 프로토타입 빈에 의존하고 있을 경우이다.**
...
public class ClientBean{
    ...
    public void logic(){
        // addcount()는 내부적으로 
        // PrototypeBean의 count필드를 +1 하는 메소드이다.
        prototypeBean.addcount();
    }
    
}
```
* ``ClientBean``은 싱글톤이므로, 보통 스프링 컨테이너 생성 시점에 함께 생성되고, 의존관계가 주입된다.
* 이때, ``ClientBean``은 의존관계 자동 주입을 사용한다. 주입 시점에서 의존하고 있는 ``prototypeBean``(프로토타입 빈)을 스프링 컨테이너에 요청할 것이다.
* 스프링 컨테이너는 ``prototypeBean``을 새로 생성해서  ``ClientBean``에 반환한다. 
* 이후에 클라이언트 1이 ``ClientBean``을 사용해서 ``clientBean.logic()``을 호출하게 되면 주입받은 ``prototypeBean``의 메서드를 호출
* 내부적으로 ``prototypeBean``안의 ``private int count``필드가 +1이 될것이다.
